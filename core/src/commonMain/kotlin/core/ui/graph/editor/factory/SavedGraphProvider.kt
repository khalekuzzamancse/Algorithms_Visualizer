...
internal object SavedGraphProvider {
    ...
    fun getTree2(nodeSizePx: Float = 48.0f): Pair<List<EditorNodeModel>, List<EditorEdgeModel>> {
        /**
         *  ------------------
         *  |   |   | 1 |   |
         *  |   | 2 |   | 3 |
         ...
         *  ------------------
         */
        val cellSize = nodeSizePx
        val _1X = cellSize * 3
        val _1Y = 0f
        val _2X = _1X - cellSize
        val _2Y = _1Y + cellSize
        val _3X = _1X + cellSize
        val _3Y = _2Y
        val _4X = _2X - cellSize
        val _4Y = _2Y + cellSize
        val _5X = _2X + cellSize
        val _5Y = _4Y
        ...
        val nodes = listOf(
            EditorNodeModel(
                id = "1",
                label = "1",
                topLeft = Offset(_1X, _1Y),
                exactSizePx = nodeSizePx
            ),
            EditorNodeModel(
                id = "2",
                label = "2",
                topLeft = Offset(_2X, _2Y),
                exactSizePx = nodeSizePx
            ),
            EditorNodeModel(
                id = "3",
                label = "3",
                topLeft = Offset(_3X, _3Y),
                exactSizePx = nodeSizePx
            ),
            EditorNodeModel(
                id = "4",
                label = "4",
                topLeft = Offset(_4X, _4Y),
                exactSizePx = nodeSizePx
            ),
            EditorNodeModel(
                id = "5",
                label = "5",
                topLeft = Offset(_5X, _5Y),
                exactSizePx = nodeSizePx
            )
        )

        // Helper function to get center position of a node by id
        fun centerOf(nodeId: String): Offset {
            val node = nodes.first { it.id == nodeId }
            return Offset(
                node.topLeft.x + node.exactSizePx / 2,
                node.topLeft.y + node.exactSizePx / 2
            )
        }

        // Define edges by connecting node ids
        val edgeConnections = listOf(
            "1" to "2",
            "2" to "4",
            "2" to "5",
            "1" to "3"
        ...
        val edges = edgeConnections.mapIndexed { index, (startId, endId) ->
            val startCenter = centerOf(startId)
            val endCenter = centerOf(endId)
            val control = Offset(
                (startCenter.x + endCenter.x) / 2,
                (startCenter.y + endCenter.y) / 2
            )
            EditorEdgeModel(
                id = (index + 1).toString(),
                start = startCenter,
