...
import graph.bfs.domain.CodeStateModel
    ...
    fun start() = sequence {
      ...
      //  yield(SimulationState.Start(code = model.toCode()))
        ...
        while (queue.isNotEmpty()) {
            ...
            yield(TraversalSimulationState.ExecutionAt(current,model.toCode()))
...
//            }
    ...
    private suspend fun SequenceScope<TraversalSimulationState>._onEdgeProcessing(
        ...
        model=model.copy( queue = "$queue")
        yield(
            TraversalSimulationState.ProcessingEdge(edgeId,model.toCode())
        )
    }
}
