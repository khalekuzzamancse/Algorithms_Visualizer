package bubble_sort.ui.visulizer.controller.ui

import androidx.compose.ui.graphics.Color
import bubble_sort.ui.visulizer.contract.AlgoVariablesState
...
internal class UIController<T : Comparable<T>>(val list: List<T>) {
    ...
    val swapPair = algoController.algoState.map { it.swappablePair }
    ...
    init {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            swapPair.collect { pair ->
                if (pair != null) {
                    arrayController.swapElement(pair.j, pair.jPlus1)
                }
            }
        }
    }
